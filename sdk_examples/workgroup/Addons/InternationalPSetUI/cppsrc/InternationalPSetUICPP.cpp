 //*****************************************************************************
/*
	InternationalPSetUI.cpp 

	This example shows how to internationalize the PPG. 
	Other implementation is same to PSetUIDemo SDK example.
...
/* namespace name */
#define XSIDICTNS L"InternationalPSetUIDemo"

/* Dictstrings */

/* Dictidstrings */
#define IDS_STATICTEXT 1
#define IDS_MSG_INSPECTOBJ 2
#define IDS_MSG_CURRRENDERPASS 3
#define IDS_MSG_TABVISIBLE 4
#define IDS_MSG_VALUEOF_NOSLIDERDEMO_PARAM 5
#define IDS_MSG_PARAMCHANGE 6
...
XSIPLUGINCALLBACK CStatus XSILoadPlugin( PluginRegistrar& in_reg )
{
	...
	// Custom Property of type "InternationalPSetUICPP".
	
	in_reg.PutAuthor( L"Autodesk,Inc." );
	in_reg.PutName( L"International PSet UI Demo Cpp Version" );
	in_reg.PutVersion( 1, 0 );

	in_reg.RegisterProperty( L"InternationalPSetUICPP" );

	// Create a menu entry in the Properties menu
	// for creating instances of this object 
	in_reg.RegisterMenu( siMenuTbGetPropertyPSetID, L"InternationalPSetUICPP_Menu", false, false ) ;
	return CStatus::OK;	
}

XSIPLUGINCALLBACK CStatus InternationalPSetUICPP_Define( const CRef & in_Ctx )
{
...
XSIPLUGINCALLBACK CStatus InternationalPSetUICPP_DefineLayout( const CRef & in_Ctx )
{
	...
	// extension.
	oLayout.PutAttribute( "Dictionary", CValue("InternationalPSetUIDemo") );
	...
	oLayout.AddTab( L"Options" ) ;
		...
		oLayout.AddGroup( L"Bitmask Demo" ) ;
			oLayout.AddRow() ;
				...
				oLayout.AddStaticText( CUtils::Translate( (LONG)IDS_STATICTEXT, XSIDICTNS ) );
	...
	oLayout.AddTab( L"Images" ) ;
		...
		oLayout.AddGroup( L"Static Bitmap Demo" ) ;
			item = oLayout.AddItem( L"StaticBitmapControl", L"Static Bitmap", siControlBitmap ) ;
				CString csLogoImage = CUtils::BuildPath( app.GetInstallationPath( siFactoryPath ), L"XSISDK/examples/workgroup/Addons/InternationalPSetUI/Data/Bitmaps/logo.bmp" );
				item.PutAttribute( siUIFilePath,  csLogoImage ) ;
		oLayout.EndGroup() ;

	return CStatus::OK;	
}

XSIPLUGINCALLBACK CStatus InternationalPSetUICPP_PPGEvent( const CRef& io_Ctx )
{
	...
	if ( eventID == PPGEventContext::siOnInit )
	{
		...
		CString csMsg = CUtils::Translate( (LONG)IDS_MSG_INSPECTOBJ, XSIDICTNS, prop.GetName(), CValue( prop.GetParameters().GetCount() ).GetAsText() );
	...
	else if ( eventID == PPGEventContext::siButtonClicked )
	{
		...
		if ( buttonPressed.GetAsText() == L"MyButton" )
		{
			...
			app.LogMessage( CUtils::Translate( (LONG)IDS_MSG_CURRRENDERPASS, XSIDICTNS, strCurrentPass ) );
	...
	else if ( eventID == PPGEventContext::siTabChange )
	{
		...
		{
			app.LogMessage( CUtils::Translate( (LONG)IDS_MSG_TABVISIBLE, XSIDICTNS ) ) ;				
		}
	}
	else if ( eventID == PPGEventContext::siParameterChange )
	{
		...
		if ( paramName == L"NoSliderDemo" )
		{
			...
			CString csMsg = CUtils::Translate( (LONG)IDS_MSG_VALUEOF_NOSLIDERDEMO_PARAM, XSIDICTNS, changed.GetValue().GetAsText() );
			app.LogMessage( csMsg ) ;
		}
		...
		app.LogMessage( CUtils::Translate( (LONG)IDS_MSG_PARAMCHANGE, XSIDICTNS, CString( L"InternationalPSetUICPP_PPGEvent" ), prop.GetName(), paramName ) );
	}

	return CStatus::OK ;
}

XSIPLUGINCALLBACK XSI::CStatus InternationalPSetUICPP_Menu_Init( XSI::CRef& in_ref )
{
	Context ctxt = in_ref;
	Menu menu = ctxt.GetSource();

	// adds a callback menu item: the supplied callback function is called
	// by Softimage when the user selects the menu item.
	MenuItem item;
	CStatus st = menu.AddCallbackItem( CUtils::Translate( L"&InternationalPSetUI - C++ API Version", XSIDICTNS), L"OnInternationalPSetUICPPMenuClicked", item );

	return st ;
}

XSIPLUGINCALLBACK XSI::CStatus OnInternationalPSetUICPPMenuClicked( XSI::CRef& )
{	
	Application app;

	CValueArray addpropArgs(5) ;
	addpropArgs[0] = L"InternationalPSetUICPP"; // Type of Property
	addpropArgs[3] = L"InternationalPSetUICPP"; // Name for the Property
