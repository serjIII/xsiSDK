// psetuidemo.js
//
// This example shows how sophisticated UI dialogs can be created 
// from scripting in Softimage using v4.0 self-installed plugin technology
//
...
function XSILoadPlugin( in_reg )
{
	...
	in_reg.Name = "PSetUIDemo JScript Version";

	// var the version number of this plugin
	in_reg.Major = 1;
	in_reg.Minor = 0 ;

	in_reg.RegisterProperty( "PSetUIDemoJScript" );
	
	// For convenience we also install a menu entry that will allow the user
	// to easily create instances of this custom property
	in_reg.RegisterMenu( siMenuTbGetPropertyPSetID,"PSetUIDemoJScript_Menu",false,false );	
}

function PSetUIDemoJScript_Define( io_Context )
{
	...
	// For the others we provide a label when we call AddItem in PSetUIDemoJScript_DefineLayout
...
function PSetUIDemoJScript_DefineLayout( io_Context )
{
		...
		oLayout.AddGroup("Current Pass Info");
			...
			oLayout.AddRow() ;
				...
				oItem = oLayout.AddItem( "CurrentPass" ) ;
					// Specify a larger value so that the button doesnt take
	...
	oLayout.AddTab( "Options" ) ;
		...
		oLayout.AddGroup( "Bitmask Demo" );
			oLayout.AddRow() ;
				...
				oLayout.AddStaticText( "Bitfields are an efficient way\r\n" +
						"to store many boolean\r\n" +
	...
	oLayout.AddTab( "Images" ) ;
		...
		oLayout.AddGroup( "Static Bitmap Demo" );
			oItem = oLayout.AddItem( "StaticBitmapControl", "Static Bitmap", siControlBitmap ) ;
				oItem.SetAttribute( siUIFilePath, Application.InstallationPath( siFactoryPath ) + "/XSISDK/examples/workgroup/Addons/PSetUIDemo/Data/Bitmaps/logo.bmp" ) ;
		oLayout.EndGroup() ;
}

function PSetUIDemoJScript_OnInit()
{
	/*
	...
	*/
	...
	for ( i = 0 ; i < PPG.Inspected.Count ; i++ )
	{
		...
		Application.Logmessage("Inspected object " + oItem.Name + " has " + oItem.Parameters.Count + " parameters" ) ;
...
function PSetUIDemoJScript_MyButton_OnClicked()
{
...
function PSetUIDemoJScript_NoSliderDemo_OnChanged()
{
...
function PSetUIDemoJScript_EnableEdit_OnChanged()
{
...
function PSetUIDemoJScript_MyRGBColorG_OnChanged()
{
...
function PSetUIDemoJScript_FileControls_OnTab()
{
	Logmessage( "File Control tab is now visible" ) ;
}

function PSetUIDemoJScript_Menu_Init( io_Context )
{
	var oMenu;
	oMenu = io_Context.Source;
	oMenu.AddCallbackItem( "PSetUIDemo - JScript Version","OnPSetUIDemoJScriptMenuClicked" );
}

function OnPSetUIDemoJScriptMenuClicked( io_Context )
{
	var oProp,oParent;
	if (Selection.Count == 0)
	{
		XSIUIToolkit.MsgBox( "Please select something and try again" ) ; 
		return;
	}
	oParent = Selection.Item( 0 );
	oProp = oParent.AddProperty( "PSetUIDemoJScript" );
	InspectObj( oProp );
	return 1;
}
