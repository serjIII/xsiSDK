...
class COGLManagedResource;

typedef struct tagGLSLUniform
{
	int			m_iSize;
	GLint		m_iID;
	float*		m_data;
...
typedef std::map<const char*, GLSLUniform*>	GLSLParameterMap;
typedef GLSLParameterMap::iterator			GLSLParameterMapIter;

class CGPUProgramGLSL 
{
public:
	...
	enum eProgramLocation
	{
		Code=0,
		File
	};

	CGPUProgramGLSL(VOID* in_pOwner);
	virtual ~CGPUProgramGLSL(void);

	virtual bool	Create		(const char* in_szVtxCode, const char* in_szFragCode);
	bool			Create		(const char* in_szCode, eProgramLocation in_eLocation);

	virtual bool	Activate	( bool in_eActivate );
	virtual bool	SetUniforms (IRTSExecutionState				* in_pExecState);
	virtual bool	DefineUniform (const char* in_szName, int in_lSize, float* in_pData, eProgramtype in_eType );
	virtual bool	DefineUniform (const char* in_szName, unsigned int in_uiValue, eProgramtype in_eType );
	virtual bool	DefineMatrixUniform (const char* in_szName, int in_iMatrixCount, float* in_pData, eProgramtype in_eType );
	virtual bool	DefineMatrixUniform (const char* in_szName, int in_iMatrixCount, double* in_pData, eProgramtype in_eType );

	virtual void*	GetUniform ( const char* in_szName );

private:
	
	BOOL						CompileAndLink ( const char* in_szCode );

	bool				IsFilenameDirty (); 
	const char*			GetCodeFromFile( const char* in_szFilename );
#ifdef unix
...
#endif
	char*				m_szCodeBuffer;
	eProgramLocation	m_location;
	VOID*	m_pOwner;



	GLhandleARB			ProgramObjectFRAG;
	GLhandleARB			ProgramObjectVTX;

	GLhandleARB			fragmentShader;
	GLhandleARB			vertexShader;

	GLSLParameterMap	m_ParamMap;
