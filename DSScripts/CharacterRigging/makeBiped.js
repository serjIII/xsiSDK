...
///////////////
/// Modified May 30 2005 Javier
/// UI additions  
///     2 new TorsoTypes --Global Axis Alignment for Torso
///     IndependentLower  Hierarchy
//
// Biped return object contains:
//
...
//	 Biped.UI.OneModel
//	 Biped.UI.IndependentLower
//	 Biped.UI.ThreeBoneToes
//
// Biped.Model
// Biped.GlobalSRT
//
...
// Biped.RKnee  	(only if joint compression option selected)
...
// Biped.LKnee  	(only if joint compression option selected)
...
//******************************************************************************
var IDS_MAKE_BIPED_TITLE			= 1164;

/*--------------------------------
 Make Biped
--------------------------------*/
function makeBiped(inTorsoType,  inTorsoStretch,  inTorsoDivisions,  inIconType,  inMakeBelly,
...
inThighSlide,  inElbowsJoint,  inKneeJoint,  inShadowType,  inShadowHands, inOneModel, inIndependentLower,
inThreeBoneToes, inCreateShadowKeySet, inInspectUI)
{

	var biped = new Object();
	biped.modelString = "Biped_Guide";
	...
	if(!biped)
	{
		...
		if(!biped)
		{
			logmessage("makeBiped: No Biped Guide Found", siError);
		}
		else	     
		{
	...
	if(!inInspectUI){
		...
		biped.UI.KneeJoint 				= inKneeJoint;
		...
		biped.UI.ThreeBoneToes			= inThreeBoneToes;
		biped.UI.CreateShadowKeySet     = inCreateShadowKeySet;
    }


	//makeBiped was cancelled from the UI
	if(!biped.UI){ logmessage("makeBiped: cancelled"); return; }
	...
	biped = generateLegs(biped);		
	oProgBar.Increment();
	if(oProgBar.CancelPressed) { Cancel(biped); return;}

	oProgBar.StatusText = "Creating Slides";		
    ...
    oProgBar.StatusText = "Creating Character Key Sets";
	...
	oProgBar.StatusText = "Applying Default Pose";				
...
function Cancel(inBiped){
	...
	logmessage("makeBiped: cancelled");
...
/*--------------------------------
 Make Guide Null
--------------------------------*/
function makeGuideNull(inParent, inName, inPos, inScale, inShape, inColor){
...
function initialHierarchy(inBiped){
    ...
    addMarkingSet(inBiped.GlobalSRT, 1, 1, 0, "Biped");

	//Create Biped Controls
	inBiped.Sliders = inBiped.model.AddProperty("Custom_parameter_list",0,"Controls");
	var layout = inBiped.Sliders.PPGLayout;

	//Set help path
	layout.SetAttribute( siUIHelpFile, "<FactoryPath>/Doc/<DocLangPref>/xsidocs.chm::/animtoolbar48.htm" );
	...
	inBiped.ControlsGroup = inBiped.model.AddGroup(aEmptyRef);


	inBiped.HiddenGroup.name = "hidden";
    inBiped.ControlsGroup.AddMember(inBiped.GlobalSRT)
	inBiped.EnvGroup.name = "envelope_group";
	inBiped.ControlsGroup.name = "controls_group";
	...
	if(inBiped.UI.ShadowType != 0){
		...
		switch (inBiped.UI.ShadowType ) {
		  	 ...
		  	 case 2: //XSI		  	 
				if (!inBiped.UI.OneModel) {
			  	 	inBiped.ShadowGlobalSRT = makeGuideNull(inBiped.ShadowModel, "GlobalSRT", zeroPos, inBiped.Scale*.07, 8, 805);
				}
				else {
			  	 	inBiped.ShadowGlobalSRT = makeGuideNull(inBiped.ShadowModel, shadowNameSuffix + "_GlobalSRT", zeroPos, inBiped.Scale*.07, 8, 805);
				}
			 break

			 case 3:
				if (!inBiped.UI.OneModel) {
					inBiped.ShadowGlobalSRT = makeGuideNull(inBiped.ShadowModel, "GlobalSRT", zeroPos, inBiped.Scale*.07, 8, 380);
				}
				else {
					inBiped.ShadowGlobalSRT = makeGuideNull(inBiped.ShadowModel, shadowNameSuffix + "_GlobalSRT", zeroPos, inBiped.Scale*.08, 8, 380);
				}
		  	 break

		 	 default:
				if (!inBiped.UI.OneModel) {
			  	 	inBiped.ShadowGlobalSRT = makeGuideNull(inBiped.ShadowModel, "GlobalSRT", zeroPos, inBiped.Scale*.07, 8, 479);
				}
				else {
			  	 	inBiped.ShadowGlobalSRT = makeGuideNull(inBiped.ShadowModel, shadowNameSuffix + "_GlobalSRT", zeroPos, inBiped.Scale*.07, 8, 479);
				}
		  	 break
		}
...
function generateTorso(inBiped){
	...
	var mirrorKeywords = new Array("Leg", "ShoulderStart", "Shoulder");
    ...
    inBiped.ControlsGroup.AddMember(inBiped.Torso.UpperBody)
    if (inBiped.UI.TorsoType != 2)
    {
        inBiped.ControlsGroup.AddMember(inBiped.Torso.HipDepth)
        inBiped.ControlsGroup.AddMember(inBiped.Torso.ChestDepth)
        inBiped.ControlsGroup.AddMember(inBiped.Torso.Hip)
        inBiped.ControlsGroup.AddMember(inBiped.Torso.Chest)
    }
    inBiped.HiddenGroup.AddMember(inBiped.Torso.RSkel(0))
    inBiped.HiddenGroup.AddMember(inBiped.Torso.LSkel(0))
...
function generateArms(inBiped){
	...
	if(inBiped.UI.RotationOrder == 0)	
	{
        ...
        inBiped.ControlsGroup.AddMember(inBiped.LArm.Skel(3))
        inBiped.ControlsGroup.AddMember(inBiped.RArm.Skel(3))
        inBiped.ControlsGroup.AddMember(inBiped.RArm.UpV)
        inBiped.ControlsGroup.AddMember(inBiped.LArm.UpV)
...
function generateHands(inBiped){
	...
	inBiped.LHand = makeHand(inBiped.UpperBody, LFingers, LBlades, LHBone, "L", inBiped.UI.FingerType,
	HandShadowType , LShadowEnd, 0);
    ...
    inBiped.ControlsGroup.AddMember(inBiped.RHand.Claw)
    inBiped.ControlsGroup.AddMember(inBiped.LHand.Claw)
...
/*--------------------------------
 Generate Legs
--------------------------------*/
function generateLegs(inBiped){

	if(inBiped.UI.ShadowType == 6 || inBiped.UI.ShadowType == 7){
		var LegShadowType = 1;
	}
	else{
		var LegShadowType = inBiped.UI.ShadowType;
	}

	// first find the leg objects from the guide

	var legItems = Array("Leg", "Knee", "Foot1");
	var lObjs = new ActiveXObject("XSI.Collection");
	var rObjs = new ActiveXObject("XSI.Collection");

	for(var i =0;i<legItems.length;i++){
		var lobj = inBiped.GuideModel.FindChild("L" + legItems[i]);
		var robj = inBiped.GuideModel.FindChild("R" + legItems[i]);
		lObjs.add(lobj);
		rObjs.add(robj);
	}

	// preset this in case it doesn't get set below
	// for an independent lower
	shadowParent = inBiped.Torso.ShadowStart;
        
	if (inBiped.UI.ShadowType){
		if (inBiped.UI.IndependentLower){
			inBiped.ShadowHip = inBiped.Torso.ShadowStart.AddNull(inBiped.Torso.HipBone.name);
            inBiped.ShadowHip.primary_icon = 4;
            inBiped.ShadowHip.size = 1.2;
			FreezeObj(inBiped.ShadowHip);
			inBiped.ShadowHip.Kinematics.AddConstraint("Pose",inBiped.Torso.HipBone);
			var display = inBiped.ShadowHip.AddProperty("Display Property")
			display.wirecol.value = 805;

			inBiped.ShadowEnvGroup.addMember(inBiped.ShadowHip)
			inBiped.Torso.ShadowStart.addChild(inBiped.ShadowHip);
			inBiped.ShadowHip.name = inBiped.Torso.HipBone.name;

			shadowParent = inBiped.ShadowHip;
		}
	}

	if(!inBiped.UI.ThighRoll){ inBiped.UI.ThighDivisions = 0}

	inBiped.LLeg = makeLeg(inBiped.GuideModel, inBiped.HipBone, "L", lObjs, 1, inBiped.UI.ThighDivisions, LegShadowType, shadowParent);
	inBiped.RLeg = makeLeg(inBiped.GuideModel, inBiped.HipBone, "R", rObjs, 1, inBiped.UI.ThighDivisions, LegShadowType, shadowParent);

	var footItems = Array("Pivot", "PivotRight", "PivotLeft", "Foot");

	// ~~~~~~~~~~~~~~make left foot~~~~~~~~~~~~~~~~~

	if(inBiped.LLeg.ShadowEnds){
		var LShadowEnd = inBiped.LLeg.ShadowEnds(0);
	}
	else{var LShadowEnd = null}
	...
	for(var i =0;i<3;i++){
		var litem = inBiped.GuideModel.FindChild("L" + footItems[i]);
		lFeet.add(litem);
	}
	for(var i=1;i<5;i++){
		var litem = inBiped.GuideModel.FindChild("L" + footItems[3] + i);
		if(!litem){break}
		lFeet.add(litem);
	}

	inBiped.LFoot = makeFoot(inBiped.GuideModel, inBiped.GlobalSRT,"L", "Left", lFeet, inBiped.Sliders, 0, inBiped.UI.ShadowType, LShadowEnd, inBiped.UI.ThreeBoneToes);

	inBiped.LLeg.Eff.AddChild(inBiped.LFoot.Root);
	inBiped.LLeg.Eff.Kinematics.AddConstraint("Position", inBiped.LFoot.BaseGuide , false);

	// ~~~~~~~~~~~~~~make right foot~~~~~~~~~~~~~~~~~

	if(inBiped.RLeg.ShadowEnds){
		var RShadowEnd = inBiped.RLeg.ShadowEnds(0);
	}
	else{var RShadowEnd = null}
	...
	for(var i =0;i<3;i++){
		var ritem = inBiped.GuideModel.FindChild("R" + footItems[i]);
		rFeet.add(ritem);
	}
	for(var i=1;i<5;i++){
		var ritem = inBiped.GuideModel.FindChild("R" + footItems[3] + i);
		if(!ritem){break}
		rFeet.add(ritem);
	}

	inBiped.RFoot = makeFoot(inBiped.GuideModel, inBiped.GlobalSRT,"R", "Right", rFeet, inBiped.Sliders, 0, inBiped.UI.ShadowType, RShadowEnd, inBiped.UI.ThreeBoneToes);

	inBiped.RLeg.Eff.AddChild(inBiped.RFoot.Root);
	inBiped.RLeg.Eff.Kinematics.AddConstraint("Position", inBiped.RFoot.BaseGuide , false);
        
        inBiped.ControlsGroup.AddMember(inBiped.RLeg.upV)
        inBiped.ControlsGroup.AddMember(inBiped.LLeg.upV)
        inBiped.ControlsGroup.AddMember(inBiped.LFoot.Foot)
        inBiped.ControlsGroup.AddMember(inBiped.LFoot.Roll)
        inBiped.ControlsGroup.AddMember(inBiped.LFoot.Tip)
        inBiped.ControlsGroup.AddMember(inBiped.LFoot.toeOffsets[0])
        inBiped.ControlsGroup.AddMember(inBiped.RFoot.Foot)
        inBiped.ControlsGroup.AddMember(inBiped.RFoot.Roll)
        inBiped.ControlsGroup.AddMember(inBiped.RFoot.Tip)
        inBiped.ControlsGroup.AddMember(inBiped.RFoot.toeOffsets[0])
...
function hookUpSliding(inBiped){
	...
	if(inBiped.UI.HipSlide){
		...
		inBiped.RHip = make2PointSlide ("RHip", RTop, RBase, inBiped.Torso.HipBone, inBiped.RLeg.Skel(1), inBiped.Torso.hipBone);
		inBiped.LHip = make2PointSlide ("LHip", LTop, LBase, inBiped.Torso.HipBone, inBiped.LLeg.Skel(1), inBiped.Torso.hipBone);
	...
	if(inBiped.UI.ThighSlide){
		...
		RSlideObjs.add(inBiped.GuideModel + ".RRear1");
		RSlideObjs.add(inBiped.GuideModel + ".RLowerThigh");
		RSlideObjs.add(inBiped.GuideModel + ".RHipBottom");
		RSlideObjs.add(inBiped.GuideModel + ".RLeg");
		RSlideObjs.add(inBiped.GuideModel + ".RKnee");

		inBiped.RThigh = makeThighSlide("R", RSlideObjs, inBiped.Torso.HipBone, inBiped.RLeg.Skel(1));



		var LSlideObjs = new ActiveXObject("XSI.Collection");

		LSlideObjs.add(inBiped.GuideModel + ".LRear1");
		LSlideObjs.add(inBiped.GuideModel + ".LLowerThigh");
		LSlideObjs.add(inBiped.GuideModel + ".LHipBottom");
		LSlideObjs.add(inBiped.GuideModel + ".LLeg");
		LSlideObjs.add(inBiped.GuideModel + ".LKnee");

		inBiped.LThigh = makeThighSlide("L", LSlideObjs, inBiped.Torso.HipBone, inBiped.LLeg.Skel(1));
...
function hookUpJoints(inBiped){
	...
	if(inBiped.UI.KneeJoint){

		var LKnee = new ActiveXObject("XSI.Collection");

		LKnee.add(inBiped.GuideModel + ".LThighBack1");
		LKnee.add(inBiped.GuideModel + ".LThighBack2");
		LKnee.add(inBiped.GuideModel + ".LCalfBack");
		LKnee.add(inBiped.GuideModel + ".LCalfCenter");

		var divisions = (inBiped.UI.LegDivisions > 0) ? inBiped.LLeg.BicepRoll.Divisions : null;
		inBiped.LKnee = makeJointCompression("LKnee", inBiped.LLeg.Skel(1), inBiped.LLeg.Skel(2), LKnee, divisions)

		var RKnee = new ActiveXObject("XSI.Collection");

		RKnee.add(inBiped.GuideModel + ".RThighBack1");
		RKnee.add(inBiped.GuideModel + ".RThighBack2");
		RKnee.add(inBiped.GuideModel + ".RCalfBack");
		RKnee.add(inBiped.GuideModel + ".RCalfCenter");

		divisions = (inBiped.UI.LegDivisions > 0) ? inBiped.RLeg.BicepRoll.Divisions : null;
		inBiped.RKnee = makeJointCompression("RKnee", inBiped.RLeg.Skel(1), inBiped.RLeg.Skel(2), RKnee, divisions)
...
function generateCharacterKeySet(inBiped){
    ...
    makeBipedCharacterKeySet(inBiped.GlobalSRT, "Biped", true, "", "");
    
    // if need be, make the shadow rig's character key set
    if (   (inBiped.UI.ShadowType > 0)
        && (inBiped.UI.CreateShadowKeySet) ) {
        makeBipedCharacterKeySet(inBiped.ShadowGlobalSRT, "Biped", true, "Shadow", "");
    }
...
function get_UI()
{
	...
	var sliders = ActiveSceneRoot.AddProperty("Custom_parameter_list",0,"Make Biped");
	...
	var KneeJoint = sliders.AddParameter2("KneeJoint",siBool,0,0,1,0,1,8,16,"Knees","Knees");
	...
	var ThreeBoneToes = sliders.AddParameter2("ThreeBoneToes",siInt4 ,0,0,1,0,1,8,16,"Type","Type");
	var ThreeBoneToesItems = Array("Three-bone toes",0,"Two-bone toes",1);
	...
	layout.EndGroup();
	layout.AddGroup("Toes");
	var item18 = layout.AddEnumControl("ThreeBoneToes",ThreeBoneToesItems,"Type",siControlRadio );
	layout.EndGroup();
	//======================= Roll Division  ============================
	layout.AddTab("Roll Division ");
	layout.AddGroup("ForeArm Roll Subdivision");
	var item19 = layout.AddItem("ForeArmRoll");
	var item20 = layout.AddItem("ForeArmDivisions");
	layout.EndGroup();
	layout.AddGroup("Bicep  Roll Subdivision");
	var item21 = layout.AddItem("BicepRoll");
	var item22 = layout.AddItem("BicepDivisions");
	layout.EndGroup();
	layout.AddGroup("Thigh  Roll Subdivision");
	var item23 = layout.AddItem("ThighRoll");
	var item24 = layout.AddItem("ThighDivisions");
	layout.EndGroup();
	//======================= Skin  ============================
	layout.AddTab("Skin ");
	layout.AddGroup("Simple Sliding");
	var item25 = layout.AddItem("ArmPitSlide");
	var item26 = layout.AddItem("HipSlide");
	layout.EndGroup();
	layout.AddGroup("Sliding and Volume Preservation");
	var item27 = layout.AddItem("ThighSlide");
	layout.EndGroup();
	layout.AddGroup("Joint Compression");
	var item28 = layout.AddItem("ElbowsJoint");
	var item29 = layout.AddItem("KneeJoint");
	layout.EndGroup();
	//======================= Shadow Rigs  ============================
	layout.AddTab("Shadow Rigs ");
	layout.AddGroup("Constrain Shadow Character");
	var item30 = layout.AddEnumControl("ShadowType",ShadowTypeItems,"Type",siControlRadio );
	layout.EndGroup();
	layout.AddGroup("Options");
	var item31 = layout.AddEnumControl("ShadowHands",ShadowHandsItems,"Hands",siControlRadio );
	var item32 = layout.AddItem("OneModel","Create Unified Skeleton and Rig Model");
	var item33 = layout.AddItem("IndependentLower","Create Independent Lower Hierarchy");
	layout.EndGroup();

	try{
		inspectobj( sliders, "", XSIUtils.Translate( IDS_MAKE_BIPED_TITLE, "XSISCRIPTS" ), siModal );
	}
	catch(e){
		deleteobj(sliders);
	...
	returnObj.KneeJoint 				= KneeJoint.value;
	...
	if (ThreeBoneToes.value == 0) {
		returnObj.ThreeBoneToes	= true;
	}
	else {
		returnObj.ThreeBoneToes = false;
	}
