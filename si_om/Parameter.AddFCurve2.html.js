var topic = "<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">\n\
<!-- saved from url=(0024)http://docs.autodesk.com -->\n\
<html>\n\
   <head>\n\
<link href=\"../style/prettify.css\" type=\"text/css\" rel=\"stylesheet\" />\n\
<script type=\"text/javascript\" src=\"../scripts/prettify.js\"></script><script src=\"../scripts/lib/jquery-1.9.1.min.js\" type=\"text/javascript\"></script><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\" /><script type=\"text/javascript\" src=\"../scripts/utils/adsk.redirect.js\"></script>\n\
      <title>Parameter.AddFCurve2</title>\n\
   \n\
</head>\n\
   <body height=\"100%\"><div class=\"body_content\" id=\"body-content\"><style>\n\
/*  These settings override ones in the default stylesheets\n\
	used by the merged files that cause problems in combination\n\
	with the standard HTML help styles and page templates. */\n\
\n\
div, div.example, div.tip {\n\
	 margin:0;\n\
}\n\
\n\
td.example {\n\
	background: inherit;\n\
}\n\
\n\
/* This fixes a problem with nested pre blocks on an example page */\n\
pre pre {\n\
	display:block;\n\
	padding:0;\n\
	border-style:none;\n\
}\n\
</style><link rel=\"stylesheet\" href=\"si_om/css/en.css\" type=\"text/css\" /><script>$(document).ready(function() { yepnope.injectJs(\"./si_om/lib/utils.js\"); });</script><script>$(document).ready(function() { yepnope.injectJs(\"./scripts/ac_common.js\"); });</script><script type=\"text/javascript\">var reflinkid = \"si_cpp\"; var reflinkdata = new Array(); function closeRefLinkDiv() { }; $(document).on(\'click\', function() { closeRefLinkDiv(); } );</script><script>$(document).ready(function() { yepnope.injectJs(\"./scripts/multireflink.js\"); });</script><script>$(document).ready(function () { prettyPrint(); } );</script><script>$(\"div#WidgetFloaterPanels,link[href*=\'microsofttranslator.com\'],script[src*=\'microsofttranslator.com\'],script[src*=\'bing.com\']\").remove();</script><script type=\'text/javascript\'>$(\"div#navigation,div#breadcrumbs,div#banner\").attr(\"translate\",\"no\"); var mtLocation = ((location && location.href && location.href.indexOf(\'https\') == 0)?\'https://ssl.microsofttranslator.com\':\'http://www.microsofttranslator.com\')+\'/ajax/v3/WidgetV3.ashx?siteData=y5CYlxTRD0znCzRLDwX0Wy7-g1EdC1XA4dSC-Y1LtaeScyli8_Ps5jPKqTr4xKxMI0OOUfkDplvX3uxN0JnPclebSYW8_J1HBzf4VLQEzQ8M4PsYXF_cMyp1Oumaetky&category=5297189e-446b-459e-ae1d-9d0360400781_tech&ctf=True&ui=true&settings=Manual&from=en&hidelanguages=\'; yepnope.injectJs(mtLocation, function() {}, { charset:\'utf-8\', type:\'text/javascript\' } );</script><script type=\"text/javascript\">\n\
 </script><!-- begin MT -->\n\
            \n\
            <div id=\'MicrosoftTranslatorWidget\' class=\'Dark\' style=\'position:absolute;right:20px;top:5px;z-index:100;color:white;background-color:#555555;height:58px;overflow:hidden\'></div><div id=\"reflinkdiv\" style=\"display:none; font-family: \'RobotoRegular\', Arial, Tahoma, sans-serif; visibility:hidden; position:absolute; z-index:40001; padding:5px; border-style:solid; border-width:1px; border-color:#666666; background-color:#dfdfdf; overflow:visible\"></div>\n\
      <div>\n\
         <div class=\"head\">\n\
            <h1>Parameter.AddFCurve2</h1>\n\
         </div>\n\
\n\
<h2 class=\"pagehead\"><a href=\"#!/url=./si_om/Parameter.html\">Parameter</a>.AddFCurve2</h2>\n\
\n\
<h3>Description</h3>\n\
<div><p> Creates and connects an <a href=\"#!/url=./si_om/FCurve.html\">FCurve</a> to this parameter. This method \n\
differs from the <a href=\"#!/url=./si_om/Parameter.AddFCurve.html\">Parameter.AddFCurve</a> method in that only the key \n\
value array and function curve type are required as arguments. The key value array \n\
contains the key frame and value only, the tangents are calculated automatically.\n\
<br /><br />\n\
If no type argument is specified, then the type of fcurve added will be based on the \n\
parameter\'s type (which is configured in the parameter\'s definition). You can override \n\
the default fcurve type associated with a parameter by adding the SPDL attribute \n\
\'FCurveType = Standard;\' or \'FCurveType = Integer;\'.\n\
<br /><br />\n\
By default standard fcurves will be created for parameters of type: siDouble, siFloat, \n\
siInt4, siInt2, siByte, siUInt4, siUInt2 and siUByte (see <a href=\"#!/url=./si_om/siVariantType.html\">siVariantType</a> \n\
for further details on types). If the parameter represents a boolean, then a boolean \n\
fcurve will be created. \n\
</p></div>\n\
\n\
<h3>C# Syntax</h3>\n\
<div><p><table>\n\
	<tr>\n\
		<td class=\"example\"><pre>FCurve Parameter.AddFCurve2( Object in_KeyValueArray, Object in_Type );</pre>		</td>	</tr>\n\
</table>\n\
</p></div>\n\
\n\
<h3>Scripting Syntax</h3>\n\
<div><p><table>\n\
	<tr>\n\
		<td class=\"example\"><pre>oReturn = Parameter.AddFCurve2( [KeyValueArray], [Type] );</pre>		</td>	</tr>\n\
</table>\n\
</p></div>\n\
\n\
<h3>Return Value</h3>\n\
<div><p><a href=\"#!/url=./si_om/FCurve.html\">FCurve</a></p></div>\n\
\n\
<h3>Parameters</h3>\n\
<div><p><table cellpadding=\"5\" cellspacing=\"5\">\n\
	<tr>\n\
		<th title=\"Name of the parameter\">Parameter		</th>\n\
		<th title=\"Data type\">Type		</th>\n\
		<th title=\"Description of the parameter\">Description		</th>\n\
	</tr>\n\
	<tr>\n\
		<td class=\"name\">KeyValueArray		</td>\n\
		<td><a href=\"#!/url=./files/Variant.htm\">Variant</a>		</td>\n\
		<td>\n\
\n\
		The array may be a 1- or 2-dimensional array. For 2 dimensional arrays\n\
		the first dimension must define the key values. The key value consists \n\
		of the key time (in frames) and the key value ( in value units ). For \n\
		bezier fcurves the tangents will be calculated automatically.\n\
			</td>\n\
	</tr>\n\
	<tr>\n\
		<td class=\"name\">Type		</td>\n\
		<td><a href=\"#!/url=./si_om/siFCurveType.html\">siFCurveType</a>		</td>\n\
		<td>\n\
The type of the FCurve.			<p class=\"cell\"><span class=\"hilite\">Default Value: </span>\n\
defined by parameter type			</p>\n\
		</td>\n\
	</tr>\n\
</table>\n\
</p></div>\n\
\n\
<h3>Examples</h3>\n\
<div><p><h4>JScript Example</h4><div class=\"example\"><table>\n\
<tr>\n\
<td class=\"example\"><pre class=\"jscript\">/*\n\
	This example demonstrates how to set up and populate an fcurve \n\
	using the z-position values from an animation fcurve created\n\
	by setting keys on a null. The newly created z-position fcurve\n\
	is then applied to a sphere\'s z-position.\n\
*/\n\
NewScene(null, false);\n\
// Get a null and animate it.\n\
var oRoot = Application.ActiveProject.ActiveScene.Root;\n\
var oNull = oRoot.AddPrimitive(\"Null\");\n\
SaveKey(oNull + \".kine.local.posx\", 1);\n\
SaveKey(oNull + \".kine.local.posy\", 1);\n\
SaveKey(oNull + \".kine.local.posz\", 1);\n\
SetValue(\"PlayControl.Current\", 50);\n\
Translate(oNull, -14, 0.5, -12, siRelative, siView, siObj, siXYZ);\n\
SaveKey(oNull + \".kine.local.posx\", 50);\n\
SaveKey(oNull + \".kine.local.posy\", 50);\n\
SaveKey(oNull + \".kine.local.posz\", 50);\n\
// Loop through the animation and store the z values into a key array for AddFCurve2 (aZKeys).\n\
var oPosVector = XSIMath.CreateVector3();\n\
var aZKeys = new Array(50 * 2+1);\n\
for ( i=0; i&lt;=50; i++ )\n\
{\n\
	var oCurrentTransform = oNull.Kinematics.Global.Transform(i);\n\
	oCurrentTransform.GetTranslation(oPosVector);\n\
	aZKeys[i*2] = i;\n\
	aZKeys[i*2+1] = oPosVector.z;\n\
}\n\
// Now get a sphere and place the FCurves on it.\n\
var oSphere = oRoot.AddGeometry(\"Sphere\", \"MeshSurface\");\n\
oSphere.Kinematics.Global.Parameters(\"posz\").AddFCurve2(aZKeys);</pre></td></tr>\n\
</table>\n\
</div></p></div>\n\
\n\
<h3>See Also</h3>\n\
<div><p><a href=\"#!/url=./si_om/Parameter.Source.html\">Parameter.Source</a> <a href=\"#!/url=./si_om/Parameter.AddFCurve.html\">Parameter.AddFCurve</a></p></div>\n\
\n\
      <div class=\"footer-block\"><a href=\"../html/ac.cmtdialog.htm\" class=\"comments-anchor\" target=\"_blank\"><span class=\"comments-link\">Please send us your comment about this page</span></a></div><br /></div>\n\
   </div></body>\n\
</html>\n\
";